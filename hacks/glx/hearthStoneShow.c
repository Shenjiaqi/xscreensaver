...
# define refresh_hearthstoneshow 0
# define release_hearthstoneshow 0
...
ENTRYPOINT ModeSpecOpt hearthstoneshow_opts = {countof(opts), opts, countof(vars), vars, NULL};
...
ENTRYPOINT void
reshape_hearthstoneshow (ModeInfo *mi, int width, int height)
{
  slideshow_state *ss = &sss[MI_SCREEN(mi)];
  GLfloat s;
...
ENTRYPOINT Bool
hearthstoneshow_handle_event (ModeInfo *mi, XEvent *event)
{
  slideshow_state *ss = &sss[MI_SCREEN(mi)];
...
check_fps (ModeInfo *mi)
{
  ...
  reshape_hearthstoneshow (mi, mi->xgwa.width, mi->xgwa.height);
#endif /* HAVE_COCOA */
}
...
ENTRYPOINT void
init_hearthstoneshow (ModeInfo *mi)
{
  ...
  if ((ss->glx_context = init_GL(mi)) != NULL) {
    reshape_hearthstoneshow (mi, MI_WIDTH(mi), MI_HEIGHT(mi));
  } else {
    MI_CLEARWINDOW(mi);
  }
...
XSCREENSAVER_MODULE_2 ("GLSlideshow", glslideshow, hearthstoneshow)
